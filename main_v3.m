...
numer_all = sum(bsxfun(@times, Y, W3), 3); 
denom_all = sum(W, 2);                     
S_hat     = bsxfun(@rdivide, numer_all, denom_all);
...
Yw = bsxfun(@times, Y, W3);        

numer_cum = cumsum(Yw, 3);         
denom_cum = cumsum(W,  2);         

S_hat_N_all = bsxfun(@rdivide, numer_cum, reshape(denom_cum, [K 1 M])); 

diff_all = bsxfun(@minus, S_hat_N_all, S_clean);
mse_all  = squeeze(mean(mean(abs(diff_all).^2, 1), 2)); 
var_emp  = mse_all(:);
